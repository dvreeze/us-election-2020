...
import election.report.extended.ExtendedTimeSeriesReport
import ujson._

/**
 * Extends reports generated from the US 2020 election voting data set with more "derived data". The input reports are
 * in the JSON format for type TimeSeriesReport. The output is the same JSON reports, yet containing more data.
 *
 * @author Chris de Vreeze
 */
object CreateExtendedReport {

  def main(args: Array[String]): Unit = {
    require(args.sizeIs == 2, s"Usage: CreateExtendedReport <json report data set> <output dir>")
    ...
    jsonReportFiles.foreach { f =>
      ...
      Try {
        ...
        val extendedReport: ExtendedTimeSeriesReport = ExtendedTimeSeriesReport.from(report)

        val extendedReportJson: Obj = extendedReport.toJsonObj

        Using.resource(new FileWriter(new File(outputDir, "extended-" + f.getName))) { fw =>
          writeTo(extendedReportJson, fw, indent = 2)
        }

        require(extendedReport.report == report, s"Removing the 'data extensions' does not yield the same as the input report")
